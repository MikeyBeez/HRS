"""Sweep: causal cumulative mean replacing attention. 4 seeds × Tiny Shakespeare.

Reuses baseline and variant_D results from the original saliency_pool sweep
(`results/`); only runs cumulative_mean here. Saves model checkpoints for
post-hoc analysis (per spec).
"""
...
SWEEP_VARIANTS = ["cumulative_mean"]
...
def main():
    ...
    ap.add_argument("--out", default="experiments/saliency_pool/results_cumulative_mean")
    ...
    for i, (v, s) in enumerate(todo):
        ...
        # Save checkpoints (per spec) so post-hoc analysis of residual stream
