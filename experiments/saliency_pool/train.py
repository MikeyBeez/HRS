...
@torch.no_grad()
def eval_ppl(model, val_data, tcfg, mcfg, device, n_batches: int = 40,
             ctx_override: int | None = None) -> float:
    model.eval()
    rng = np.random.default_rng(1234)
    losses = []
    ctx = ctx_override or mcfg.ctx_len
    for _ in range(n_batches):
        x, y = sample_lm_batch(val_data, tcfg.batch_size, ctx, device, rng)
        logits = model(x)
...
def train_one(variant: str, seed: int, out_dir: Path,
    ...
    # Long-context generalization probe is skipped: the model's pos_emb is
    # sized to ctx_len, so evaluating beyond it would use untrained position
    # embeddings (noise, equally bad for all variants — uninformative).
    long_ppl = None
    long_ctx = None
    peak_mem = (
    ...
    record = {
        ...
        "long_ctx_val_ppl": long_ppl,
        "long_ctx_len": long_ctx if long_ppl is not None else None,
    ...
    long_str = f" long_ppl={long_ppl:.3f}" if long_ppl is not None else ""
    print(f"  [{variant}/seed{seed}] DONE val_ppl={final_ppl:.3f}{long_str} "
          f"params={record['total_params']:,} "
          f"output/layer={record['output_path_params_per_layer'][0]:,} "
