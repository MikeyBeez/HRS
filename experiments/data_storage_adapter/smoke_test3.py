"""Smoke test 3: push rank and step count to find the single-passage ceiling
on tiny-shakespeare base with single-layer FFN LoRA."""
...
def show_per_para(model, entries, tokenizer, device, cfg, lora_scale=1.0):
    for e in entries:
        for p in e["paraphrases_held_out"]:
            ...
            out = generate_completion(model, ids_t, max_new_tokens=20,
...
def main():
    ...
    # Push capacity higher.
    configs = [
        {"rank": 128, "n_steps": 800,  "high_lr": 5e-3, "base_lr": 1e-4, "temperature": 0.6, "top_k": 20},
        {"rank": 256, "n_steps": 800,  "high_lr": 5e-3, "base_lr": 1e-4, "temperature": 0.6, "top_k": 20},
        {"rank": 256, "n_steps": 1500, "high_lr": 5e-3, "base_lr": 1e-4, "temperature": 0.6, "top_k": 20},
        {"rank": 256, "n_steps": 1500, "high_lr": 5e-3, "base_lr": 1e-4, "temperature": 0.3, "top_k": 5},
    ]
    for c in configs:
        print(f"\n--- {c} ---")
        t0 = time.time()
